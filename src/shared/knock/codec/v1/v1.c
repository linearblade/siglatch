...
static int shared_knock_codec_v1_packet_nonce_accept(SharedKnockCodecV1State *state,
                                                      uint32_t timestamp,
                                                      uint32_t challenge);
static int shared_knock_codec_v1_copy_recv_packet(const SharedKnockNormalizedUnit *src,
                                                   M7MuxRecvPacket *dst);
static int shared_knock_codec_v1_copy_send_packet(const M7MuxSendPacket *src,
...
int shared_knock_codec_v1_copy_user_recv_data(M7MuxUserRecvData *dst,
...
size_t shared_knock_codec_v1_count_fragments(const void *state_,
  ...
  return (payload_len <= SHARED_KNOCK_CODEC_V1_PAYLOAD_MAX) ? 1u : 0u;
}
...
int shared_knock_codec_v1_encode(const void *state_,
  ...
  session = context ? context->openssl_session : NULL;
  capacity = *out_len;
  ...
  wire_pkt.action_id = normal->action_id;
  ...
  should_auth = normal->wire_auth ? 1 : 0;
  if (should_auth) {
    if (!session) {
      return 0;
    }
    ...
    memcpy(wire_pkt.hmac, normal->hmac, sizeof(wire_pkt.hmac));
  }
...
static size_t shared_knock_codec_v1_adapter_count_fragments(const M7MuxContext *ctx,
                                                            const void *state,
  ...
  if (!shared_knock_codec_v1_copy_send_packet(send, &normal)) {
    return 0u;
  }

  return shared_knock_codec_v1_count_fragments((const SharedKnockCodecV1State *)state,
                                               &normal,
                                               0u);
}
...
static int shared_knock_codec_v1_adapter_decode(const M7MuxContext *ctx,
  ...
  if (!shared_knock_codec_v1_copy_recv_packet(&normal, out)) {
    return 0;
  }
  if (out && ingress) {
...
static const M7MuxNormalizeAdapter shared_knock_codec_v1_adapter = {
  ...
  .alloc_user_recv_data = shared_knock_codec_v1_alloc_user_recv_data,
  .free_user_recv_data = shared_knock_codec_v1_free_user_recv_data,
  .copy_user_recv_data = shared_knock_codec_v1_copy_user_recv_data,
  .count_fragments = shared_knock_codec_v1_adapter_count_fragments,
