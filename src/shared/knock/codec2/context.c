...
static void shared_knock_codec_context_free_server_key(SharedKnockCodecServerKey *entry);
static void shared_knock_codec_context_free_entry(SharedKnockCodecKeyEntry *entry);
static int shared_knock_codec_context_copy_server_key(const SharedKnockCodecServerKey *src,
                                                      SharedKnockCodecServerKey *dst);
static int shared_knock_codec_context_copy_entry(const SharedKnockCodecKeyEntry *src,
                                                 SharedKnockCodecKeyEntry *dst);
static int shared_knock_codec_context_ensure_capacity(SharedKnockCodecContext *context,
                                                      size_t needed);
static int shared_knock_codec_context_name_exists(const SharedKnockCodecContext *context,
                                                  const char *name);
...
int shared_knock_codec_context_create(SharedKnockCodecContext **out_context) {
  SharedKnockCodecContext *context = NULL;

  if (!g_initialized || !out_context) {
    return 0;
  }

  context = (SharedKnockCodecContext *)calloc(1u, sizeof(*context));
  if (!context) {
    return 0;
  }
...
void shared_knock_codec_context_destroy(SharedKnockCodecContext *context) {
  ...
  shared_knock_codec_context_free_server_key(&context->server_key);
  context->openssl_session = NULL;

  for (i = 0; i < context->keychain_len; ++i) {
    shared_knock_codec_context_free_entry(&context->keychain[i]);
  }
...
int shared_knock_codec_context_set_server_key(SharedKnockCodecContext *context,
  ...
  SharedKnockCodecServerKey copy = {0};

  if (!g_initialized || !context || !server_key || !server_key->name || server_key->name[0] == '\0') {
    return 0;
  }

  if (!shared_knock_codec_context_copy_server_key(server_key, &copy)) {
    return 0;
  }
...
int shared_knock_codec_context_add_keychain(SharedKnockCodecContext *context,
  ...
  SharedKnockCodecKeyEntry *slot = NULL;
  size_t index = 0;
  ...
  if (shared_knock_codec_context_name_exists(context, entry->name)) {
    return 0;
  }

  if (!shared_knock_codec_context_ensure_capacity(context, context->keychain_len + 1u)) {
    return 0;
  }

  index = context->keychain_len;
  slot = &context->keychain[index];
  if (!shared_knock_codec_context_copy_entry(entry, slot)) {
    shared_knock_codec_context_free_entry(slot);
    return 0;
  }
...
int shared_knock_codec_context_remove_keychain(SharedKnockCodecContext *context,
  ...
  for (i = 0; i < context->keychain_len; ++i) {
    SharedKnockCodecKeyEntry *entry = &context->keychain[i];
    ...
    shared_knock_codec_context_free_entry(entry);
...
static int shared_knock_codec_context_copy_server_key(const SharedKnockCodecServerKey *src,
  ...
  if (src->public_key) {
    if (EVP_PKEY_up_ref((EVP_PKEY *)src->public_key) != 1) {
      shared_knock_codec_context_free_server_key(dst);
      return 0;
    }
...
static int shared_knock_codec_context_copy_entry(const SharedKnockCodecKeyEntry *src,
  ...
  if (src->public_key) {
    if (EVP_PKEY_up_ref((EVP_PKEY *)src->public_key) != 1) {
      shared_knock_codec_context_free_entry(dst);
      return 0;
    }
    dst->public_key = src->public_key;
  }
...
static int shared_knock_codec_context_ensure_capacity(SharedKnockCodecContext *context,
                                                      size_t needed) {
  SharedKnockCodecKeyEntry *entries = NULL;
  size_t old_cap = 0u;
  size_t new_cap = 0u;
  size_t bytes = 0u;
  ...
  entries = (SharedKnockCodecKeyEntry *)realloc(context->keychain, bytes);
  if (!entries) {
    return 0;
  }
...
static const SharedKnockCodecContextLib _instance = {
