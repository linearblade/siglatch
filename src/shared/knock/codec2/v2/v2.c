...
struct SharedKnockCodec2V2State {
  NonceCache nonce;
  int nonce_ready;
...
static const SharedKnockCodecContext *shared_knock_codec2_v2_context(void);
static int shared_knock_codec2_v2_sync_nonce_cache(SharedKnockCodec2V2State *state);
static int shared_knock_codec2_v2_private_decrypt(const uint8_t *input,
                                                  size_t input_len,
                                                  uint8_t *output,
                                                  size_t *output_len);
static int shared_knock_codec2_v2_packet_nonce_accept(SharedKnockCodec2V2State *state,
                                                      uint32_t timestamp,
                                                      uint32_t challenge);
...
static time_t shared_knock_codec2_v2_nonce_ttl(void) {
  const SharedKnockCodecContext *context = shared_knock_codec2_v2_context();
...
static int shared_knock_codec2_v2_sync_nonce_cache(SharedKnockCodec2V2State *state) {
  ...
  cfg.ttl_seconds = shared_knock_codec2_v2_nonce_ttl();
...
static int shared_knock_codec2_v2_packet_nonce_accept(SharedKnockCodec2V2State *state,
  ...
  if (!shared_knock_codec2_v2_sync_nonce_cache(state)) {
    return 0;
  }
...
int shared_knock_codec2_v2_create_state(SharedKnockCodec2V2State **out_state) {
  SharedKnockCodec2V2State *state = NULL;

  if (!out_state) {
    return 0;
  }

  state = (SharedKnockCodec2V2State *)calloc(1u, sizeof(*state));
  if (!state) {
    return 0;
  }

  if (!shared_knock_codec2_v2_sync_nonce_cache(state)) {
    shared_knock_codec2_v2_destroy_state(state);
    *out_state = NULL;
    return 0;
  }
...
int shared_knock_codec2_v2_detect(const SharedKnockCodec2V2State *state,
  ...
  if (buflen == SHARED_KNOCK_V2_FORM1_PACKET_SIZE &&
      shared_knock_v2_form1_codec_deserialize(buf, buflen, &pkt) == SL_PAYLOAD_OK) {
    if (state) {
      ((SharedKnockCodec2V2State *)state)->last_packet_encrypted = 0;
    }
    return 1;
  }
  ...
  }
  {
    ...
    if (!shared_knock_codec2_v2_private_decrypt(buf, buflen, decrypted, &decrypted_len)) {
      free(decrypted);
      return 0;
    }

    if (decrypted_len != SHARED_KNOCK_V2_FORM1_PACKET_SIZE) {
      free(decrypted);
      return 0;
    }

    if (shared_knock_v2_form1_codec_deserialize(decrypted, decrypted_len, &pkt) != SL_PAYLOAD_OK) {
      free(decrypted);
      return 0;
    }
  ...
  ((SharedKnockCodec2V2State *)state)->last_packet_encrypted = 1;
  return 1;
}
...
int shared_knock_codec2_v2_decode(const SharedKnockCodec2V2State *state,
  ...
  if (should_decrypt) {
    ...
    size_t decrypted_len = decrypted_cap;
    if (!shared_knock_codec2_v2_private_decrypt(buf,
                                                buflen,
                                                decrypted_buf,
                                                &decrypted_len)) {
      free(decrypted_buf);
  ...
  rc = shared_knock_v2_form1_codec_normalize(payload, payload_len, ip, client_port, should_decrypt, out);
  if (rc != SL_PAYLOAD_OK) {
    if (should_decrypt) {
      free((void *)payload);
    }
  ...
  if (!shared_knock_codec2_v2_packet_nonce_accept((SharedKnockCodec2V2State *)state,
                                                  out->timestamp,
                                                  out->challenge)) {
    if (should_decrypt) {
      free((void *)payload);
...
int shared_knock_codec2_v2_encode(const SharedKnockCodec2V2State *state,
                                  ...
                                  size_t *out_len) {
  (void)state;
  return shared_knock_codec2_v1_encode(NULL, normal, out_buf, out_len);
}

static const SharedKnockCodec2V2Lib shared_knock_codec2_v2 = {
  .name = "v2",
