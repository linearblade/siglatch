...
static int m7mux_stream_is_expired(const M7MuxNormalizedPacket *slot, uint64_t now_ms) {
...
static int m7mux_stream_ingest(M7MuxStreamState *state, const M7MuxNormalizedPacket *normal) {
  M7MuxNormalizedPacket *slot = NULL;
  size_t i = 0;
  ...
  if (slot->session_id == 0u) {
    ...
    if (slot->payload_len > sizeof(slot->payload_buffer)) {
      memset(slot, 0, sizeof(*slot));
      return 0;
    }

    if (slot->payload_len > 0u) {
      memcpy(slot->payload_buffer, normal->payload_buffer, slot->payload_len);
    }
  ...
  if (slot->payload_len + normal->payload_len > sizeof(slot->payload_buffer)) {
    return 0;
  }

  memcpy(slot->payload_buffer + slot->payload_len,
         normal->payload_buffer,
         normal->payload_len);
  slot->payload_len += normal->payload_len;
  ...
  slot->user_id = normal->user_id;
  slot->action_id = normal->action_id;
  slot->challenge = normal->challenge;
  memcpy(slot->hmac, normal->hmac, sizeof(slot->hmac));
  memcpy(slot->response_buffer, normal->response_buffer, sizeof(slot->response_buffer));
  slot->response_len = normal->response_len;
...
static int m7mux_stream_drain(M7MuxStreamState *state, M7MuxNormalizedPacket *out_normal) {
  size_t i = 0;
  M7MuxNormalizedPacket *slot = NULL;
...
static int m7mux_stream_expire(M7MuxStreamState *state, uint64_t now_ms) {
  ...
  for (i = 0; i < M7MUX_STREAM_READY_QUEUE_CAPACITY; ++i) {
    M7MuxNormalizedPacket *slot = &state->ready_queue[i];
