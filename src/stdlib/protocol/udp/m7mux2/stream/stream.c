...
static int m7mux2_stream_init(void) {
  return 1;
}

static void m7mux2_stream_shutdown(void) {
}

static int m7mux2_stream_state_init(M7Mux2StreamState *state) {
  if (!state) {
    return 0;
  }
...
static void m7mux2_stream_state_reset(M7Mux2StreamState *state) {
  if (!state) {
    return;
  }
...
static int m7mux2_stream_has_pending(const M7Mux2StreamState *state) {
  if (!state) {
    return 0;
  }
...
static int m7mux2_stream_is_expired(const M7Mux2RecvPacket *slot, uint64_t now_ms) {
  ...
  return (now_ms - slot->received_ms) >= M7MUX2_STREAM_EXPIRE_AFTER_MS;
}

static int m7mux2_stream_ingest(M7Mux2StreamState *state, const M7Mux2RecvPacket *normal) {
  M7Mux2RecvPacket *slot = NULL;
  size_t i = 0;
  ...
  for (i = 0; i < M7MUX2_STREAM_READY_QUEUE_CAPACITY; ++i) {
...
static int m7mux2_stream_drain(M7Mux2StreamState *state, M7Mux2RecvPacket *out_normal) {
  size_t i = 0;
  M7Mux2RecvPacket *slot = NULL;
...
static int m7mux2_stream_expire(M7Mux2StreamState *state, uint64_t now_ms) {
  size_t i = 0;
  int expired = 0;
  ...
  for (i = 0; i < M7MUX2_STREAM_READY_QUEUE_CAPACITY; ++i) {
    M7Mux2RecvPacket *slot = &state->ready_queue[i];

    if (!m7mux2_stream_is_expired(slot, now_ms)) {
      continue;
    }
...
static int m7mux2_stream_pump(M7Mux2StreamState *state, uint64_t now_ms) {
  return m7mux2_stream_expire(state, now_ms);
}

static const M7Mux2StreamLib _instance = {
  .init = m7mux2_stream_init,
  .shutdown = m7mux2_stream_shutdown,
  .state_init = m7mux2_stream_state_init,
  .state_reset = m7mux2_stream_state_reset,
  .has_pending = m7mux2_stream_has_pending,
  .ingest = m7mux2_stream_ingest,
  .drain = m7mux2_stream_drain,
  .expire = m7mux2_stream_expire,
  .pump = m7mux2_stream_pump
};
