...
static void m7mux2_inbox_reset_context(void) {
  memset(&g_ctx, 0, sizeof(g_ctx));
}

static int m7mux2_inbox_init(const M7Mux2Context *ctx) {
  if (!ctx) {
    m7mux2_inbox_reset_context();
    return 0;
  }
...
static int m7mux2_inbox_set_context(const M7Mux2Context *ctx) {
  return m7mux2_inbox_init(ctx);
}

static void m7mux2_inbox_shutdown(void) {
  m7mux2_inbox_reset_context();
}

static void m7mux2_inbox_state_reset(M7Mux2State *state) {
  if (!state) {
    return;
  }
...
static int m7mux2_inbox_state_init(M7Mux2State *state) {
  if (!state) {
    return 0;
  }

  if (!g_ctx.internal->session->state_init(&state->session)) {
    m7mux2_inbox_state_reset(state);
    return 0;
  }
...
static int m7mux2_inbox_has_pending(const M7Mux2State *state) {
  if (!state) {
    return 0;
  }
...
static int m7mux2_inbox_pump(M7Mux2State *state, uint64_t timeout_ms) {
  M7Mux2Ingress raw = {0};
...
static int m7mux2_inbox_drain(M7Mux2State *state, M7Mux2RecvPacket *out_normal) {
  if (!state || !out_normal) {
    return 0;
  }
...
static const M7Mux2InboxLib _instance = {
  .init = m7mux2_inbox_init,
  .set_context = m7mux2_inbox_set_context,
  .shutdown = m7mux2_inbox_shutdown,
  .state_init = m7mux2_inbox_state_init,
  .state_reset = m7mux2_inbox_state_reset,
  .has_pending = m7mux2_inbox_has_pending,
  .pump = m7mux2_inbox_pump,
  .drain = m7mux2_inbox_drain
};
