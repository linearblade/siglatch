...
#include "../../../shared/knock/codec3/v1/v1_packet.h"
#include "../../../stdlib/base64.h"

static int app_daemon4_payload_reply_action_prefix(uint8_t action_id,
                                                   char *out,
                                                   size_t out_size);
static int app_daemon4_payload_build_semantic_reply(
    const AppConnectionJob *job,
    const AppActionReply *reply,
    uint8_t *out_buf,
    size_t out_size,
    size_t *out_len);
static int app_daemon4_payload_stage_reply(
    AppRuntimeListenerState *listener,
    SiglatchOpenSSLSession *session,
    AppConnectionJob *job,
    const AppActionReply *reply);
static int app_daemon4_payload_base64_size(size_t input_len, size_t *out_size);
static int app_daemon4_payload_dispatch_action(
    AppRuntimeListenerState *listener,
    const AppConnectionJob *job,
    SiglatchOpenSSLSession *session,
...
static int app_daemon4_payload_init(void) {
  return 1;
}

static void app_daemon4_payload_shutdown(void) {
}
...
static int app_daemon4_payload_build_semantic_reply(
  ...
  *out_len = 0;
  ...
  if (app_daemon4_payload_reply_action_prefix(job->request.action_id,
                                              action_prefix,
  ...
  if (max_text_len > (SHARED_KNOCK_CODEC3_V1_PAYLOAD_MAX - SL_KNOCK_RESPONSE_PAYLOAD_HEADER_SIZE)) {
    max_text_len = SHARED_KNOCK_CODEC3_V1_PAYLOAD_MAX - SL_KNOCK_RESPONSE_PAYLOAD_HEADER_SIZE;
  }
...
static int app_daemon4_payload_consume(AppRuntimeListenerState *listener,
                                       AppConnectionJob *job,
                                       SiglatchOpenSSLSession *session) {
  ...
  if (job->request.payload_len > 0u && !job->request.payload_buffer) {
    LOGE("[daemon4.payload] Missing payload buffer for non-empty job (user_id=%u action_id=%u)\n",
         job->request.user_id,
  ...
  if (job->wire_version == 0u) {
    LOGD("[daemon4.payload] Routing raw unstructured payload to fallback handler\n");
  ...
  if (!app.daemon4.request.bind_user_action(listener, job, session, &user, &action)) {
    return 0;
  }

  if (!app.daemon4.policy.enforce(listener, job, user, action)) {
    return 0;
  }

  return app_daemon4_payload_dispatch_action(listener, job, session, user, action, job);
}
...
static int app_daemon4_payload_stage_reply(
  ...
  if (!app.daemon4.job.reserve_response(job, APP_JOB_RESPONSE_BLOCK_SIZE)) {
    job->should_reply = 0;
    return 0;
  }

  memset(job->response_buffer, 0, job->response_cap);
  if (!app_daemon4_payload_build_semantic_reply(job,
                                                reply,
                                                job->response_buffer,
                                                job->response_cap,
                                                &response_len)) {
...
static int app_daemon4_payload_dispatch_action(
  ...
  LOGD("[daemon4.payload] Routing normalized unit to action handlers\n");

  if (!listener || !listener->server || !job || !session || !user || !action || !out_job) {
    LOGE("[daemon4.payload] Invalid arguments to dispatch_action\n");
    return 0;
  }
  ...
  if (app.builtin.is_action(action)) {
    ...
    if (!app.builtin.build_context(
      ...
      LOGE("[daemon4.payload] Failed to build builtin context for action (%s)\n", action->name);
      return 0;
    }

    LOGD("[daemon4.payload] Routing to builtin: %s (Ip=%s, User=%s, Action=%s)\n",
         action->builtin,
         job->ip,
         user->name,
         action->name);
    ...
    if (!ok) {
      if (builtin_reply.should_reply) {
        ...
        if (!reply_user || !app.inbound.crypto.assign_session_to_user(session, reply_user)) {
          LOGE("[daemon4.payload] Failed to rebuild user crypto context for builtin error reply (%s)\n",
               action->name);
          return 0;
        }
      }

      if (!app_daemon4_payload_stage_reply(listener, session, out_job, &builtin_reply)) {
        LOGE("[daemon4.payload] Builtin error reply stage failed for action (%s)\n", action->name);
        return 0;
      }
      return 0;
    }
    ...
    if (builtin_reply.should_reply) {
      reply_user = app.config.user_by_id(job->request.user_id);
      if (!reply_user || !app.inbound.crypto.assign_session_to_user(session, reply_user)) {
        LOGE("[daemon4.payload] Failed to rebuild user crypto context for builtin reply (%s)\n",
             action->name);
        return 0;
      }
    }
    ...
    if (!app_daemon4_payload_stage_reply(listener, session, out_job, &builtin_reply)) {
      LOGE("[daemon4.payload] Builtin reply stage failed for action (%s)\n", action->name);
      return 0;
    }
  ...
  if (action->handler == SL_ACTION_HANDLER_STATIC ||
    ...
    if (!app.object.build_context(
      ...
      LOGE("[daemon4.payload] Failed to build object context for action (%s)\n", action->name);
      return 0;
    }

    LOGD("[daemon4.payload] Routing to %s object: %s (Ip=%s, User=%s, Action=%s)\n",
    ...
    if (!app_daemon4_payload_stage_reply(listener, session, out_job, &object_reply)) {
      LOGE("[daemon4.payload] Object reply stage failed for action (%s)\n", action->name);
      return 0;
    }
  ...
  if (action->constructor[0] == '\0') {
    LOGW("[daemon4.payload] Shell action ID %u has no constructor\n", job->request.action_id);
    return 0;
  }
  ...
  if (job->request.payload_len > 0u) {
    if (!app_daemon4_payload_base64_size(job->request.payload_len, &payload_b64_size)) {
      LOGE("[daemon4.payload] Shell payload too large for base64 buffer (payload_len=%zu)\n",
           job->request.payload_len);
      app.payload.reply.set(&shell_reply, 0, "ERROR %s payload_too_large", action->name);
      if (!app_daemon4_payload_stage_reply(listener, session, out_job, &shell_reply)) {
        LOGE("[daemon4.payload] Shell overflow reply stage failed for action (%s)\n", action->name);
      }
      return 0;
    }
    ...
    if (!payload_b64) {
      LOGE("[daemon4.payload] Failed to allocate base64 buffer for shell payload (payload_len=%zu)\n",
           job->request.payload_len);
      app.payload.reply.set(&shell_reply, 0, "ERROR %s payload_alloc_failed", action->name);
      if (!app_daemon4_payload_stage_reply(listener, session, out_job, &shell_reply)) {
        LOGE("[daemon4.payload] Shell alloc-failure reply stage failed for action (%s)\n",
             action->name);
      }
      return 0;
    }
  ...
  LOGD("[daemon4.payload] Routing to script: %s (Ip=%s, User=%s, Action=%s, execSplit=%d)\n",
       action->constructor,
       job->ip,
       user->name,
       action->name,
  ...
  if (!app.payload.run_shell_wait(
    ...
    if (!app_daemon4_payload_stage_reply(listener, session, out_job, &shell_reply)) {
      LOGE("[daemon4.payload] Shell error reply stage failed for action (%s)\n", action->name);
    }
  ...
  if (!app_daemon4_payload_stage_reply(listener, session, out_job, &shell_reply)) {
    LOGE("[daemon4.payload] Shell reply stage failed for action (%s)\n", action->name);
    if (payload_b64_allocated) {
      free(payload_b64);
    }
    return 0;
...
static const AppDaemon4PayloadLib app_payload_instance = {
  .init = app_daemon4_payload_init,
  .shutdown = app_daemon4_payload_shutdown,
  .consume = app_daemon4_payload_consume
};
